			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder') {
						...
						if (value.isenable == 0)
						{
							$entity.find('.tree-branch-name > .tree-label').html('<font color="#FF0000">'+value.text || value.name+'</font>');
						}
						else {
							$entity.find('.tree-branch-name > .tree-label').html('<font color="#000000">'+value.text || value.name+'</font>');
						}
						...
						if (value.isenable == 0)
						{
							$entity.find('.tree-item-name > .tree-label').html('<font color="#FF0000">'+value.text || value.name+'</font>');
						}
						else {
							$entity.find('.tree-item-name > .tree-label').html('<font color="#000000">'+value.text || value.name+'</font>');
						}
